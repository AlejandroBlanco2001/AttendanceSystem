...
const Contract = () => {
  ...
  const addContract = () => {
    axios
      .post("http://localhost:80/admin/create/contracts", {
        id_stud,
        code_syll,
        year_peri,
        term_peri,
      ...
      .catch((er) => {
        alert(
          "You`re trying to add a contract whose primary key is already in existance."
        );
        console.log(er);
      });
  };
  ...
  const deleteContract = (id) => {
    axios
      .post(
        `http://localhost:80/admin/delete/contracts/${id[0]}|${id[1]}`,
        { 1: true },
